...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(503)
        .json({ error: `Could not add the short: ${error}` })
        .end();
    }
  }
);
...
router.get("/", [], async (req, res) => {
  try {
    ...
    res
      .status(503)
      .json({ error: `Could not fetch all shorts: ${error}` })
      .end();
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(503)
        .json({ error: `Could not update the short: ${error}` })
        .end();
    }
  }
);
...
router.delete(
  "/:name?",
  [v.ensureUserLoggedIn, v.ensureValidShortNameParam],
  async (req, res) => {
    try {
      // middleware will ensure that a valid user is logged in
