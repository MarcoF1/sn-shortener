...
class Shorts {
  ...
  static async addOne(shortName, url, creatorID) {
    ...
    return db
      .run(`INSERT INTO shorts VALUES ('${shortName}', '${url}', ${creatorID})`)
      .then(() => {
  ...
  static async findOne(shortName) {
    return db.get(
      `SELECT * FROM shorts WHERE ${db.columnNames.shortName} = '${shortName}'`
    );
  }

  /**
   * Find a Short's URL by Name.
   *
   * @param {string} shortName - name of Short to find
   * @return {string | undefined} - found Url
   */
  static async findUrl(shortName) {
    return db.get(
      `SELECT url FROM shorts WHERE ${db.columnNames.shortName} = '${shortName}'`
    );
  }
  ...
  static async deleteOne(shortName) {
    ...
    return Shorts.findOne(shortName).then((short) => {
      db.run(
        `DELETE FROM shorts WHERE ${db.columnNames.shortName} = '${shortName}'`
      );
      return short;
    });
  }
